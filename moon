...
local src = game:HttpGet("https://raw.githubusercontent.com/LovelyHub-Cranel/lua./refs/heads/main/librarydnm")

src = src:gsub("SidebarCompacted%s*=%s*false", "SidebarCompacted = true")
src = src:gsub("SidebarCompactWidth%s*=%s*%d+", "SidebarCompactWidth = 40")
src = src:gsub("CompactWidthActivation%s*=%s*%d+", "CompactWidthActivation = 90")

local Library = loadstring(src)()

local ThemeManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/LovelyHub-Cranel/lua./refs/heads/main/theme"))()
...
sound.Volume  = 1
sound.SoundId = "rbxassetid://121944056716897"

Library:Notify({
    Title       = "Moon",
    Description = "Script Açıldı",
    BigIcon     = "rbxassetid://",
    Time        = 4,
})
sound:Play()
...
local Tabs = {
    ...
    Defence = Window:AddTab("Defence", "moon"),
    Target = Window:AddTab("Target", "heart-pulse"),
    Blobman = Window:AddTab("Blobman", "heart"),
    Main = Window:AddTab("Main", "candy"),
...
do
local Box = Tabs.Defence:AddRightGroupbox("Defence", "wrench")
local bOx = Tabs.Defence:AddRightGroupbox("Moon Defence", "heart")
local box = Tabs.Defence:AddLeftGroupbox("Defence", "shield")
...
sound.Parent  = workspace
sound.Volume  = 1
sound.SoundId = "rbxassetid://134724353558032"
...
box:AddToggle("AntiInput", {
    ...
    Callback = function(v)
        ...
        if antiinputlag then
            local burger = inv:FindFirstChild("FoodHamburger") or spawntoy("FoodHamburger", HRP.CFrame)
            burger.Name = "burger"
            task.wait(0.1)
            spawn(function()
                while antiinputlag and task.wait() do
                    ...
                    if (burger.HoldPart.RigidConstraint.Attachment1 and burger.HoldPart.RigidConstraint.Attachment1 ~= plr.Character["Left Arm"].LeftGripAttachment) or (not burger or not burger.Parent) then
                        ...
                        burger = spawntoy("FoodHamburger", HRP.CFrame)
...
Box:AddToggle("GucciTractor", {
    ...
    Callback = function(v)
        if v then
            ...
            sno(blobb.Part)
            task.wait(0.25)
            blobb.VehicleSeat.CFrame = CFrame.new(0, 100000000, 0)
        else
...
Box:AddToggle("GucciBlobman", {
    ...
    Callback = function(v)
        if v then
            ...
            plr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
            task.wait()
            blobb.VehicleSeat.CFrame = CFrame.new(0, 10000000, 0)
        else
				...
				local function StickKunai(kunai)
					...
					for _, obj in pairs(kunai:GetChildren()) do
						...
						elseif obj.Name == "Main" then
							...
							if not obj:FindFirstChild("Highlight") then
								local high = Instance.new("Highlight", obj)
								high.FillColor = Color3.fromRGB(95, 10, 10)
							end
...
local ToyList = {
    ["PalletLightBrown"] = "PalletLightBrown",
	["TableBenchSmall"] = "ChildrensTableBenchSmall",
	["DiceSmall"]   = "DiceSmall",
	["JapaneseTable"]   = "JapaneseTable",
	["TableSmall"] = "TableSmallLabTable",
	["TableWood"] = "TableWoodFourLegsBrown",
	["TableWoodTwo"] = "TableWoodTwoLegs",
}
...
--// DROPDOWN
Box:AddDropdown("AntiKickToy", {
	Text = "Anti Kick Loop Item",
	Values = DropdownValues,
	Default = 1,
	Callback = function(Value)
		SelectedToy = ToyList[Value]
		print("SelectedToy:", SelectedToy)
	end
})

local forceSpawn = false

bOx:AddToggle("ForceSpawn", {
	Text = "Anti Kick Loop",
	Default = false,
	Callback = function(v)
		forceSpawn = v
	...
	while true do
		...
		if forceSpawn then
			...
			local success, err = pcall(function()
				if HRP and SelectedToy then
					spawntoy(SelectedToy, HRP.CFrame * CFrame.new(0,-1,20))
				end
			end)
    ...
    Callback = function(v)
        ...
        if antikick then
            task.spawn(function()
                ...
                if not inv:FindFirstChild("BoxCrateWood") then
                    ...
                    while antikick do
                        ...
                        pcall(function()
                            ...
                            if not shu 
                                or not inv:FindFirstChild("BoxCrateWood") 
                                and not workspace.PlotItems:FindFirstChild("BoxCrateWood", true) then
                                
                                print("Spawn")

                                shu = spawntoy("BoxCrateWood", HRP.CFrame * CFrame.new(0, 0, 20))
                                if shu then
                                    shu.Name = "BoxCrateWood"
                                    part = shu:FindFirstChild("StickyPart")
                                    if part then
                                        sno(part)
                                    end
                                end
                            ...
                            if shu and part 
                                and part:FindFirstChild("PartOwner") 
                                and part.PartOwner.Value ~= plr.Name then
                                
                                
                                sno(part)
                            end

                            if part and part:FindFirstChild("StickyWeld") then
                                local target = char:FindFirstChild("HumanoidRootPart")
                                if target and part.StickyWeld.Part1 ~= target then
                                    
                                    sno(part)
                                    StickyEvent:FireServer(part, target, CFrame.new())
                                end
                            end

                            if shu and part and (part.Position - HRP.Position).Magnitude > 30 then
                                

                                local tool = inv:FindFirstChild("BoxCrateWood")
                                if tool then
                                    DestroyToy:FireServer(tool)
                                end
...
local player = Players.LocalPlayer
local dialogueParent = player:WaitForChild("PlayerGui")

local floppadialogo = Instance.new("ScreenGui")
local Floppa = Instance.new("ImageLabel")
local Bubble_chat = Instance.new("ImageLabel")
local BubbleTextchat = Instance.new("TextLabel")

local typingsoundeffect = Instance.new("Sound", workspace)
local typingsoundeffect2 = Instance.new("Sound", workspace)
...
Floppa.BackgroundTransparency = 1
Floppa.Image = "rbxthumb://type=Asset&id=10110319532&w=420&h=420"
Floppa.Size = UDim2.new(0.19,0,0.30,0)
Floppa.Position = UDim2.new(0.02,0,2,0)
...
Bubble_chat.Size = UDim2.new(1.03,0,0.79,0)
Bubble_chat.Position = UDim2.new(0.68,0,-0.29,0)
...
BubbleTextchat.Size = UDim2.new(0.63,0,0.26,0)
BubbleTextchat.Position = UDim2.new(0.18,0,0.36,0)
BubbleTextchat.Text = ""
...
local floppatween = TweenService:Create(
    Floppa,
    tweenInfo,
    {Position = UDim2.new(0.02,0,0.66,0)}
)

local floppamessageoncooldown = false
...
function Antivoidmesssage()
    ...
    if not floppamessageoncooldown then
        ...
        floppatween.Completed:Connect(function()

            Bubble_chat.Visible = true
            BubbleTextchat.Visible = true

            local dialogueText = "Seni Femboylardan Kurtardım"

            for i = 1,#dialogueText do
                BubbleTextchat.Text = string.sub(dialogueText,1,i)
...
local MESSAGE_COOLDOWN = 2.5   


local lastMessageTime = 0

local StruggleEvent = ReplicatedStorage:FindFirstChild("CharacterEvents", true) 
                     and (ReplicatedStorage.CharacterEvents:FindFirstChild("Struggle") or ReplicatedStorage.CharacterEvents:FindFirstChild("ForceRelease") or ReplicatedStorage:FindFirstChild("Struggle", true))

local antiHighConn
local struggleSpamConn
...
local function forceToGround(root, hum, reason)
    if not root then return end
    
    local safeCFrame = CFrame.new(root.Position.X, GROUND_Y, root.Position.Z) * root.CFrame.Rotation
    ...
    Antivoidmesssage(reason or "")
end

local function startStruggleSpam()
    if struggleSpamConn then struggleSpamConn:Disconnect() end
    
    struggleSpamConn = RunService.Heartbeat:Connect(function()
        local char = Player.Character
        if not char then return end
        
        local isHeld = Player:FindFirstChild("IsHeld") or Player:FindFirstChild("Grabbed") 
        if isHeld and isHeld.Value == true then
            ...
            if root then
                ...
                pcall(function()
                    ReplicatedStorage.GrabEvents.SetNetworkOwner:FireServer(root, nil)  -- Server'a bırakmaya çalış
                end)
            end
...
local function highAndGrabCheck()
    if not isEnabled then return end
    
    local root, hum = getRootAndHum()
    if not root or not hum then return end
    
    
    if root.Position.Y > MAX_SAFE_HEIGHT then
        forceToGround(root, hum, "")
    end
    ...
    if isHeld and isHeld.Value then
        root.AssemblyLinearVelocity = Vector3.new(
            root.AssemblyLinearVelocity.X * 0.05,
            -150, 
            root.AssemblyLinearVelocity.Z * 0.05
        )
        Antivoidmesssage("")
    end
    
    
    local blackHole = workspace:FindFirstChild("BlackHole") or workspace:FindFirstChildWhichIsA("Part", true) -- isim değişebilir
    if blackHole and blackHole.Name:lower():find("black") and (root.Position - blackHole.Position).Magnitude < 60 then
        forceToGround(root, hum, "")
    end
end

-- Toggle
bOx:AddToggle("AntiGround", {
    Text = "Anti-Ground",
    Default = false,
    Callback = function(enabled)
        ...
        if enabled then
            if antiHighConn then antiHighConn:Disconnect() end
            antiHighConn = RunService.Heartbeat:Connect(function(dt)
                if tick() % CHECK_INTERVAL < dt * 2 then
                    pcall(highAndGrabCheck)
                end
            end)
            ...
            Player.CharacterAdded:Connect(function()
                if isEnabled then
                    task.wait(0.6)
                    local r, h = getRootAndHum()
                    if r then forceToGround(r, h, "") end
                    startStruggleSpam()
                end
            end)
            
        else
            if antiHighConn then antiHighConn:Disconnect() antiHighConn = nil end
            if struggleSpamConn then struggleSpamConn:Disconnect() struggleSpamConn = nil end
        end
    end
})
...
local autoStruggleConn = nil
local lastStruggleTime = 0
local lastAnchorTime = 0


local STRUGGLE_COOLDOWN = 0.26     
local ANCHOR_DEBOUNCE   = 0.38     
local VELOCITY_DOWN     = -72      

bOx:AddToggle("AntiLoopKick", {
    Text = "Anti-LoopKick",
    Default = false,
    Callback = function(Value)
        if autoStruggleConn then
        ...
        autoStruggleConn = RunService.Heartbeat:Connect(function()
            ...
            if isGrabbed then
                ...
                if Struggle and tick() - lastStruggleTime > STRUGGLE_COOLDOWN then
                    lastStruggleTime = tick()
                    pcall(function()
                ...
                pcall(function()
                    ...
                    if gce then
                        local stopVel = gce:FindFirstChild("StopAllVelocity")
                        if stopVel then
                            stopVel:FireServer()
                        end
                    end
                ...
                if tick() - lastAnchorTime > ANCHOR_DEBOUNCE then
                    lastAnchorTime = tick()
                    ...
                    local root = char:FindFirstChild("HumanoidRootPart")
                    if root then
                        root.AssemblyLinearVelocity = Vector3.new(0, VELOCITY_DOWN, 0)
                ...
                if tick() - lastAnchorTime > 0.45 then
                    for _, part in pairs(char:GetChildren()) do
                        if part:IsA("BasePart") then
                            part.Anchored = false
                        end
...
local plr = Players.LocalPlayer
local spawnRemote = ReplicatedStorage.MenuToys.SpawnToyRemoteFunction
local stickyEvent = ReplicatedStorage.PlayerEvents.StickyPartEvent
local setOwner = ReplicatedStorage.GrabEvents.SetNetworkOwner

_G.KatanaAnti = _G.KatanaAnti or false
...
local function getKatana()
	...
	return inv:FindFirstChild("Anti") or inv:FindFirstChild("NinjaKatana")
end

local function spawnKatana()
	local hrp = getHRP()
	pcall(function()
...
local function forceOwner(katana)
	local sound = katana:FindFirstChild("SoundPart")
	if sound then
		pcall(function()
...
local function stickKatana(katana)

	local sticky = katana:FindFirstChild("StickyPart")
	local hrp = getHRP()
	...
	if sticky and firePart then
		pcall(function()
			stickyEvent:FireServer(
				sticky,
				firePart,
				CFrame.new() * CFrame.Angles(0,math.rad(90),math.rad(90))
			)
		end)
	end
...
local function protectParts(katana)
...
task.spawn(function()
	...
	while task.wait(0.03) do
		...
		if sticky then
			...
			if not weld or sticky.CanTouch then
				stickKatana(katana)
			end

			if (getHRP().Position - sticky.Position).Magnitude > 15 then
				stickKatana(katana)
			end
...
local tpActive = false
bOx:AddToggle("AntiKatana", {
    Text = "Anti-Katana",
    Default = false,
    Callback = function(Value)
		_G.KatanaAnti = Value
		local function ClearKatana()
			local plr = game.Players.LocalPlayer
			local inv = workspace:FindFirstChild(plr.Name .. "SpawnedInToys")
			local destroyrem = game.ReplicatedStorage:FindFirstChild("MenuToys") and game.ReplicatedStorage.MenuToys:FindFirstChild("DestroyToy")
			if inv and destroyrem then
				for _, v in pairs(inv:GetChildren()) do
					if v.Name == "Anti" or v.Name == "NinjaKatana" then
						pcall(function()
							destroyrem:FireServer(v)
						end)
					end
		...
		if Value then
			task.spawn(function()
				...
				local function StickKatana(katana)
					if not katana or not katana:FindFirstChild("StickyPart") then
						return
					end
					local currentHRP = getHRP()
					if not currentHRP then
					...
					if katana:FindFirstChild("SoundPart") then
						if not katana.SoundPart:FindFirstChild("PartOwner") or katana.SoundPart.PartOwner.Value ~= plr.Name then
							setOwner:FireServer(katana.SoundPart, katana.SoundPart.CFrame)
						end
					end
					local firePart = currentHRP:FindFirstChild("FirePlayerPart") or currentHRP:WaitForChild("FirePlayerPart", 5)
					if firePart then
					...
					for _, obj in pairs(katana:GetChildren()) do
						if obj.Name == "Pyramid" then
							obj.CanTouch = false;
							obj.CanCollide = false;
							obj.CanQuery = false;
						...
						elseif obj.Name == "Main" then
							...
							if not obj:FindFirstChild("Highlight") then
								local high = Instance.new("Highlight", obj)
								high.FillColor = Color3.fromRGB(124, 213, 133)
							end
						elseif obj:IsA("BasePart") then
							obj.CanTouch = false;
							obj.CanCollide = false;
				...
				local function SpawnToy(name)
					local t = tick()
					while not canSpawn.Value do
						if not _G.KatanaAnti or tick() - t > 1 then
							return nil
						end
						task.wait(0.01)
					end
					...
					if currentHRP then
						task.spawn(function()
							pcall(function()
								spawnRemote:InvokeServer(name, currentHRP.CFrame * CFrame.new(0, -1, 20), Vector3.new(0, 0, 0))
							end)
						end)
					end
					local boolik, house = CheckForHome()
				...
				while _G.KatanaAnti do
                    ...
                    pcall(function()
	                    local inv = workspace:FindFirstChild(plr.Name .. "SpawnedInToys")
	                    if inv then
		                    local kat = inv:FindFirstChild("Anti") or inv:FindFirstChild("NinjaKatana")
		                    if kat and kat:FindFirstChild("StickyPart") then
			                    StickKatana(kat)
		                    end
	                    end
                    end)
					...
					local inv = workspace:FindFirstChild(plr.Name .. "SpawnedInToys")
					local katana = inv and inv:FindFirstChild("NinjaKatana")
					if workspace.PlotItems.PlayersInPlots:FindFirstChild(plr.Name) then
						local boolik, house = CheckForHome()
						if boolik and house and workspace.Plots:FindFirstChild(house.Name) then
							local sign = workspace.Plots[house.Name]:FindFirstChild("PlotSign")
							if sign and sign.ThisPlotsOwners.Value.TimeRemainingNum.Value > 89 then
								katana = SpawnToy("NinjaKatana")
								if katana == nil then
									continue
								end
								katana.Name = "Anti"
					...
					repeat
						if katana and katana:FindFirstChild("StickyPart") and katana.StickyPart.CanTouch == true then
							StickKatana(katana)
							katana.Name = "Anti"
						end
						task.wait(0.3)
					until not katana or not _G.KatanaAnti
							or not katana:FindFirstChild("StickyPart")
							or katana.StickyPart.CanTouch == false 
							or not plr.Character or not plr.Character:FindFirstChild("HumanoidRootPart") 
							or not katana:FindFirstChild("StickyPart") 
							or (plr.Character.HumanoidRootPart.Position - katana.StickyPart.Position).Magnitude >= 20
					if not katana or not katana:FindFirstChild("StickyPart") or not plr.Character or not plr.Character:FindFirstChild("HumanoidRootPart") or (plr.Character.HumanoidRootPart.Position - katana.StickyPart.Position).Magnitude >= 20 then
						ClearKatana()
					end
					pcall(function()
						repeat
							task.wait(0.01)
						until not _G.KatanaAnti or not plr.Character or not plr.Character:FindFirstChild("Humanoid") or not katana or not katana:FindFirstChild("StickyPart") or not katana.StickyPart:FindFirstChild("StickyWeld") or not katana.StickyPart.StickyWeld.Part1
						if not katana or not katana:FindFirstChild("StickyPart") or (plr.Character and plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.Health <= 0) or not katana["StickyPart"]:FindFirstChild("StickyWeld").Part1 then
							ClearKatana()
						end
					end)
				end
...
Floppa.Parent = floppadialogo
Floppa.BackgroundTransparency = 1
Floppa.Image = "rbxthumb://type=Asset&id=16765815633&w=420&h=420"
Floppa.Size = UDim2.new(0.19,0,0.30,0)
Floppa.Position = UDim2.new(0.02,0,2,0)
...
function antivoidmesssage()
    ...
    if not floppamessageoncooldown then
        ...
        floppatween.Completed:Connect(function()
            ...
            local dialogueText = "Seni Anime Kızlarından Kurtardım"
...
local box = Tabs.Home:AddLeftGroupbox("Script", "user")

end

do

local box = Tabs.Main:AddLeftGroupbox("Combat", "hand")
...
local function playKickSound()
	...
	s.SoundId = "rbxassetid://76443999837674" -- Bell (Deltarune)
	s.Volume = 5
	s.PlayOnRemove = true
...
do
local box = Tabs.Blobman:AddLeftGroupbox("Selected", "user")

local bOx = Tabs.Blobman:AddRightGroupbox("Blobman", "target")

local boX = Tabs.Blobman:AddLeftGroupbox("Settings", "wrench")
...
bOx:AddDropdown("Method", {
    Text = "Method",
    Values = {"Kick", "Lock Target1", "Lock Target2", "Super Lock", "Kill","Fast Kill"},
    Default = 0,
    Multi = false,
    Callback = function(v)
        Sets.Method = v
...
boX:AddToggle("EnableBlobmanAntiKick", {
    Text = "Enable Anti Anti Kick",
    Default = false,
    Callback = function(v)
        if v then
            task.spawn(function()
                while Toggles.EnableBlobmanAntiKick.Value and RunService.RenderStepped:Wait() do
                    if not BlobmanTarget.Value then return end
                    local tt = Players:FindFirstChild(getname(BlobmanTarget.Value))
                    if not tt then return end
...
do
local box = Tabs.Target:AddLeftGroupbox("Selected", "user")

local Box = Tabs.Target:AddRightGroupbox("No Blobman", "target")
...
Box:AddDropdown("Method", {
    Text = "Method",
    Values = {"Loop Grab(Kick)","Fast Grab(Kick)", "Loop Grab", "Kill","Fast Kill","House Unlock"},
    Default = 0,
    Multi = false,
    Callback = function(v)
        Sets.Method = v
            ...
            task.spawn(function()
                while applymethod and RunService.RenderStepped:Wait() do
                    ...
                    if tt and tt.Character and tt.Character:FindFirstChild("HumanoidRootPart") and HRP then
                        ...
                        elseif Method == "House Unlock" then
                            if Sets.Char.Parent ~= workspace then
                                local pos = HRP.CFrame
                        ...
                        elseif Method ~= "Loop Grab(Kick)" then
                            if Sets.HRP:FindFirstChild("BodyPosition") then Sets.HRP.BodyPosition:Destroy() 
                            end
                        end
                    end
                end
...
do
local box = Tabs.Visual:AddRightGroupbox("Lags")
local lps = 1000
local Packets = 3000
box:AddSlider("LPS", {
...
box:AddToggle("PacketLag", {
    ...
    Callback = function(v)
        ...
        if v then
            task.spawn(function()
                while PacketsEnabled and task.wait(0.25) do
                    if AntiDetect then
...
box:AddToggle("HeartSparklerBuild", {
	Text = "Heart",
	Default = false,
	Callback = function(Value)
		heartHighRun = Value
...
box:AddToggle("RagdollGlitchLeft", {
    Text = "Ragdoll Glitch Left",
    Default = false,
    Callback = function(Value)
        if Value then
            local Remote = game.ReplicatedStorage.CharacterEvents.RagdollRemote
            local Character = game.Players.LocalPlayer.Character or game.Players.LocalPlayer.CharacterAdded:Wait()
            local OldVoid = workspace.FallenPartsDestroyHeight
            local Position = Character.HumanoidRootPart.CFrame
            workspace.FallenPartsDestroyHeight = -100
            local Leg = "Left Leg" -- veya "Right Leg"

            Remote:FireServer(Character.HumanoidRootPart, 2)
            task.wait(0.5)
            Character[Leg].CFrame = CFrame.new(0, -10000, 0)
            task.wait(0.3)
            Character.Torso.CFrame = CFrame.new(0, -9970, 0)
            task.wait(0.5)
            Character.Torso.CFrame = Position
            workspace.FallenPartsDestroyHeight = OldVoid
            Remote:FireServer(Character.HumanoidRootPart, 1)
        end
    end
})
box:AddToggle("RagdollGlitchRight", {
    Text = "Ragdoll Glitch Right",
    Default = false,
    Callback = function(Value)
        if Value then
            local Remote = game.ReplicatedStorage.CharacterEvents.RagdollRemote
